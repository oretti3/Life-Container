...
**"A secure, Dockerized sandbox for your AI Life-Agent."**

Life-Containerは、Google公式の **Gemini CLI** を搭載した、あなた専用の人生管理エージェント実行環境です。
GitHub CLI (`gh`) との連携が完了したDockerコンテナを提供し、ホスト環境を汚すことなく、AIに自律的なタスク管理、コード生成、リポジトリ操作を依頼できます。

## 🌟 特徴

* **公式 Gemini CLI 搭載:** Googleアカウントでログインするだけで、無料枠（Gemini 1.5 Flashなら1日1500回）を利用可能。APIキー管理は不要です。
* **完全なサンドボックス:** ホストOSから隔離された環境。AIがコマンド操作を誤っても安全です。
* **GitHub Native:** 起動時に指定リポジトリを自動Clone（未存在なら作成）。Issue管理やPR作成が即座に行えます。
* **ホスト連携:** UID 1000で動作するため、生成されたファイルはホスト側（VS Code等）から権限エラーなしで編集可能です。
...
```bash
    docker compose up
```
初回起動時、自動的にGitHub認証が行われ、必要なリポジトリが `workspace/` に準備されます。
...
```bash
    ...
    # ワンライナー（コマンド実行や要約）
    gemini "GitHubのIssueリストを確認して、今日やるべきことを教えて"
    
    # ファイル操作を含む指示
    gemini "README.mdの誤字を修正して"
```
